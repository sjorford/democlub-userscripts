...
(function() {
	...
	function shortOrgName(text) {
		var shortName = text.trim().replace(/( local| mayoral)? election| Mayoral Election$/, '');
		if (shortName == 'City of London Corporation' || shortName == 'City of London') return 'City of London';
		return _shortName(shortName.replace(/^The /, ''));
	}
	
	function shortPostName(text) {
		return _shortName(text).replace(/ ward$/g, '');
	}
	...
	function _shortName(text) {
		return text
				...
				.replace(/^Police and Crime Commissioner for (.*) (Police|Constabulary)$/, '$1')
