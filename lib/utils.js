...
(function() {
	...
	function shortOrgName(text) {
		var shortName = text.trim().replace(/( local| mayoral)? election$/, '');
		if (shortName == 'City of London Corporation' || shortName == 'City of London') return 'City of London';
		return shortName.replace(/^(The )?(Borough of |Borough Council of |London Borough of |Royal Borough of |City of |City and County of |Council of the |Comhairle nan )?(.+?)(( County| County Borough| Metropolitan Borough| Borough| Metropolitan District| District| City and District| City)? Council| Combined Authority| ward)?$/, '$3').replace(/ City(?! Region)/g, '');
	}
